...
def backtest(
    ...
    pnl = 0
    max_pnl = 0
    trade_side = 0  # 0 no positon , 1 long, -1 short
    entry_price = None
    max_drawdown = 0
    ...
    for i in range(len(highs)):
        ...
        for side in ["resistances", "supports"]:
            ...
            if row["rounded_" + h_l][i] in price_groups[side]:
                ...
                if grp["start_time"] == None:
                    for c in last_hl[side]:
                ...
                if broken_in_last < 3 and (
                    ...
                    if len(grp["prices"]) >= min_points:
                        ...
                        levels[side].append(
                            [(grp["start_time"], extreme_price), (index, extreme_price)]
                        )

                        resistances_supports[side].append(
                            {"price": extreme_price, "broken": False}
                        )
                ...
                if broken_in_last < 3:
                    price_groups[side][row["rounded_" + h_l][i]] = {
                        "prices": [row[h_l][i]],
                        "start_time": index,
                        "last": index,
                    }
            ...
            for sup_res in resistances_supports[side]:
                entry_condition = (
                    ...
                    if side == "resistances"
                    else row["close"][i] < sup_res["price"]
                )
            ...
            # Check PNL
            if trade_side == 1:
                if row["close"][i] >= entry_price * (1 + take_profit / 100) or row[
                    "close"
                ][i] <= entry_price * (1 - stop_loss / 100):
    ...
    #     type="candle",
    #     style="charles",
    #     alines=dict(alines=levels["resistances"] + levels["supports"]),
    # )
    # plt.show()
